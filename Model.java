...
class Model {
    ...
    Model(Model model) {
        ...
        _history.addAll(model._history);
        _score = 0; _streakLength = 0;
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        if (piece.height() + row <= _height
                && piece.width() + col <= _width) {
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int bonus = _width * nrows + _height * ncols;
        if (nrows >= 1 && ncols >= 1) {
            bonus = _width * nrows + _height * ncols
                    - Math.abs(nrows + ncols - 1);
            return bonus + _width * nrows + _height * ncols;
        }
        if (_streakLength > 0) {
            bonus = _streakLength * (_width * nrows + _height * ncols);
        }
        return bonus + _width * nrows + _height * ncols;
    }
    ...
    boolean get(int row, int col) {
        if (!_cells[row][col] && isCell(row, col)) {
            return false;
        }
        return true;
    }
    ...
    protected ArrayList<Piece> _hand = new ArrayList<>();
